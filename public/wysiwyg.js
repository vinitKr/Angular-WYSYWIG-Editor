...
var editorTemplate = "<div class=\"tinyeditor\">" +
	...
	"<textarea data-placeholder-attr=\"\" style=\"-webkit-box-sizing: border-box; -moz-box-sizing: border-box; box-sizing: border-box; resize: none; width: 100%; height: 100%;cursor: pointer\" ng-show=\"editMode\" ng-model=\"content\"></textarea>" +
...
angular.module('ngWYSIWYG').directive('wysiwygEdit', ['$compile', '$timeout', '$q', '$http', '$window',
    function($compile, $timeout, $q, $http, $window) {
	var linker = function( scope, $element, attrs, ctrl ) {
		...
		addBlank: "<div class=\"tinyeditor-control\" title=\"Blank Page\" style=\"background-position: 34px -120px;\"><img id=\"save\" src=\"images/page_2.png\" ng-click=\"addBlank(content)\"></div>",
		addNew: "<div class=\"tinyeditor-control\" title=\"Add New Page\" style=\"background-position: 34px -120px;\"><img id=\"save\" src=\"images/document_add.png\" ng-click=\"addNew(content)\"></div>",
		save: "<div class=\"tinyeditor-control\" title=\"Save\" style=\"background-position: 34px -120px;\"><img id=\"save\" src=\"images/save.png\" ng-click=\"save(content)\"></div>",
		...
		{ name: 'basicStyling', items: ['addBlank', 'addNew', 'save', '-', 'bold', 'italic', 'underline', 'strikethrough', 'subscript', 'superscript', 'leftAlign', 'centerAlign', 'rightAlign', 'blockJustify', '-'] },
	    ...
	    */
		...
		// }
		// else {
		    val = prompt('Please enter the picture URL');
		    if(val){
		    	val = '<img src="' + val + '" style="width: 30%;">'; //we convert into HTML element.
			} else{
				val = ' ';
			}
		// }
	    ...
	    */
	    scope.save = function(data) {
	    	var content ={data: '<div class=\"container\">' + data + '</div>'};
	    	$http.post('/save', content).success(function(response) {
          console.log(response);
        })
	    }
	    
	    scope.addBlank = function(data) {
	    	// var old = data;
	    	var val = confirm('Do you really want to clear the field without Saving ?')
	     	if(val){
	     		scope.content= '';
	     	}
	    }
	    ...
	    scope.addNew = function(data) {
	    	var val = confirm('Do you save want Save the changes ?')
	     	if(val){
	    		scope.save(data);
	     	}
